...
func resourceArmDnsCNameRecordCreateOrUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := dnsClient.CreateOrUpdate(ctx, resourceGroup, zoneName, name, dns.CNAME, parameters, eTag, ifNoneMatch)
	if err != nil {
		return err
	}

	if resp.ID == nil {
		return fmt.Errorf("Cannot read DNS CNAME Record %s (resource group %s) ID", name, resourceGroup)
	}
...
func resourceArmDnsCNameRecordRead(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := dnsClient.Get(ctx, resourceGroup, zoneName, name, dns.CNAME)
	if err != nil {
...
func resourceArmDnsCNameRecordDelete(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := dnsClient.Delete(ctx, resourceGroup, zoneName, name, dns.CNAME, "")
